...
class VCockpitPanel extends HTMLElement {
    ...
    registerInstrument(_instrumentName, _instrumentClass) {
        ...
        let instrument = this.createInstrument(_instrumentName, _instrumentClass);

        handler = new YourControlsHandler(instrument.instrumentIdentifier, instrument.isInteractive)
    }
...
Coherent.on("OnInteractionEvent", function (_target, _args) {
    if (!closed) {
        ...
        if (panel) {
            ...
            // Returns whether to send data to instruments
            if (!handler.processInteractionEvent(_args)) { return }
